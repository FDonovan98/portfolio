...
During a consultation with other artists on our team, a bad habit I have developed was highlighted, which was always trying to make an asset out of a single mesh. While I have found modelling an asset from a single mesh does reduce the likelihood of producing faces overlapping each other, which could cause artifacting and take up additional unneeded space when UVing, a section of a model can often be easier and quicker to model individually rather than manipulating existing geometry. This single-mesh-manipulation can also produce undesirable topology compared to modelling parts separately, potentially increasing the polycount of a model significantly. As such, I aim to actively consider whether a part of my model would be better to model separately moving forward, as while I am certain there are use cases for this method, it should not be the method I <i>always</i> use.
